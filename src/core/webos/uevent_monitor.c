...
struct SDL_webOSUeventMonitor
{
    ...
    const char *prefix;    // "hidraw", "event", "js" or "video"
    const char *subsystem; // "hidraw", "input" or "video4linux"
    int log_category;
...
static bool ParseUevent(char *buf, size_t len, const char **subsystem, const char **devname,
                        dev_t *devnum, SDL_webOSUeventAction *action);
...
static void EmitEvent(SDL_webOSUeventMonitor *monitor, SDL_webOSUeventAction action, const char *name,
                      dev_t devnum, SDL_webOSUevent *event);
...
SDL_webOSUeventMonitor *SDL_webOSUeventMonitorOpen(SDL_webOSDevicePresenceCheck watch)
{
    ...
    int log_category = SDL_LOG_CATEGORY_INPUT;
    int fd;
    ...
    case SDL_WEBOS_DEVICE_PRESENCE_CHECK_JS:
        ...
        subsystem = "input";
        break;
    case SDL_WEBOS_DEVICE_PRESENCE_CHECK_VIDEO:
        base_dir = "/dev";
        prefix = "video";
        subsystem = "video4linux";
        log_category = SDL_LOG_CATEGORY_SYSTEM;
        break;
    default:
    ...
    if (fd < 0) {
        SDL_LogWarn(log_category, "Unable to open netlink uevent socket: %s", strerror(errno));
        return NULL;
    }
    ...
    monitor->log_category = log_category;
...
bool SDL_webOSUeventMonitorPoll(SDL_webOSUeventMonitor *monitor, SDL_webOSUevent *event)
{
    ...
    for (;;) {
        ...
        if (monitor->queue_count > 0) {
            ...
            EmitEvent(monitor, (SDL_webOSUeventAction)queued->action, queued->name, 0, event);
            return true;
        }
        ...
        if (bytes <= 0) {
            ...
            if (bytes < 0 && errno == ENOBUFS) {
                ...
                SDL_LogWarn(monitor->log_category,
        ...
        if (msg.msg_flags & MSG_TRUNC) {
            SDL_LogWarn(monitor->log_category, "Discarding truncated uevent (%d bytes)", (int)bytes);
            continue;
        }

        monitor->buf[bytes] = '\0';

        if (!ParseUevent(monitor->buf, (size_t)bytes, &subsystem, &devname, &devnum, &action)) {
            continue;
        }
        ...
        // A node name is only meaningful within its own subsystem.
        if (monitor->subsystem != NULL &&
            (subsystem == NULL || SDL_strcmp(subsystem, monitor->subsystem) != 0)) {
            continue;
        }
        ...
        EmitEvent(monitor, action, devname, devnum, event);
        return true;
    }
}
...
static void Resynchronize(SDL_webOSUeventMonitor *monitor)
{
    ...
    if (dir == NULL) {
        SDL_LogWarn(monitor->log_category, "Unable to enumerate %s: %s",
                        ...
                        dev_t *devnum, SDL_webOSUeventAction *action)
{
    ...
    *devnum = 0;
    ...
    while (pos < len) {
        ...
        if (SDL_strncmp(field, "ACTION=", 7) == 0) {
            ...
            *subsystem = field + 10;
            ...
            node = field + 8;
        } else if (SDL_strncmp(field, "MAJOR=", 6) == 0) {
            major_str = field + 6;
        } else if (SDL_strncmp(field, "MINOR=", 6) == 0) {
            minor_str = field + 6;
        }
    ...
    /* Left at 0 unless both halves parse; no character device is 0:0. */
    if (major_str != NULL && minor_str != NULL) {
        char *major_end = NULL;
        char *minor_end = NULL;
        unsigned long major_num = SDL_strtoul(major_str, &major_end, 10);
        unsigned long minor_num = SDL_strtoul(minor_str, &minor_end, 10);

        if (major_end != major_str && *major_end == '\0' &&
            minor_end != minor_str && *minor_end == '\0') {
            *devnum = makedev((unsigned int)major_num, (unsigned int)minor_num);
        }
    }
