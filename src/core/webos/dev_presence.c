...
Uint32 SDL_webOSGetDevicePresenceFlags(SDL_webOSDevicePresenceCheck check)
{
    ...
    for (int dev_idx = 0; dev_idx < dev_count; dev_idx++) {
        ...
        dev_num = SDL_strtol(dev_list[dev_idx]->d_name + prefix_len, &endptr, 10);
        if (endptr == NULL || *endptr != '\0') {
            free(dev_list[dev_idx]); // SHOULD NOT be freed with SDL_free()
            continue;
        }
        SDL_snprintf(dev_path, sizeof(dev_path), "%s/%s", base_dir, dev_list[dev_idx]->d_name);
...
bool SDL_webOSIsDeviceIndexPresent(Uint32 flags, int index)
{
...
static NodeLiveness CheckSysDevChar(dev_t devnum)
{
    ...
    SDL_snprintf(path, sizeof(path), "/sys/dev/char/%u:%u", major(devnum), minor(devnum));
 ...
 * /sys/class/hidraw or /sys/class/video4linux to match it, so this answers
 * for evdev and js only. Anything else would never be listed here, and
 * reading that as absent would hide every such node in a production jail. */
static NodeLiveness CheckSysClassInput(const char *name)
{
    char path[128];
    struct stat st;
    ...
    SDL_snprintf(path, sizeof(path), "/sys/class/input/%s", name);
...
static int is_hidraw(const struct dirent *dir)
{
    return dir->d_type == DT_CHR && SDL_strncmp(dir->d_name, "hidraw", 6) == 0;
}

static int is_evdev(const struct dirent *dir)
{
    return dir->d_type == DT_CHR && SDL_strncmp(dir->d_name, "event", 5) == 0;
}

static int is_jsdev(const struct dirent *dir)
{
    return dir->d_type == DT_CHR && SDL_strncmp(dir->d_name, "js", 2) == 0;
}
